...
class Portfolio(BaseModel):
    ...
    def create_empty_portfolio(self, recommendation_set : object):
        ...
        for security in securities:
            ...
            (price_date_str, latest_price) = intrinio_data.get_latest_close_price(ticker, datetime.now(), 5)
        ...
        try:
            ...
            raise ValidationError("Could parse price date returned by Intrinio API", e)
        ...
        log.info("Created empty portfolio using price date of: %s", str(price_date))
    ...
    def reprice(self, price_date : datetime):
        ...
        for security in self.model['securities_set']:
            ...
            (price_date_str, latest_price) = intrinio_data.get_latest_close_price(security['ticker_symbol'], price_date, 5)
            security['current_price'] = latest_price
            security['current_returns'] = ((latest_price / analysis_price) - 1)
        ...
        if not self.is_empty():
            for security in self.model['current_portfolio']['securities']:
                ...
                (price_date_str, latest_price) = intrinio_data.get_latest_close_price(security['ticker_symbol'], price_date, 5)
                security['current_price'] = latest_price
                security['current_returns'] = ((latest_price / purchase_price) - 1)
